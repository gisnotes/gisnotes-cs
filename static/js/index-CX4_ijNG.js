import{c as G,o as I,C as r,D as k}from"./cesium-DrqHkmj_.js";import{C as P}from"./cesium-pNEfRpPT.js";import{C as V}from"./gui-lg9F6Ebh.js";import{_ as H,r as W,T as $,F as Q,M as q,o as J,m as K,f as rr,h as er,i as nr,H as tr}from"./index-BZhg8qW3.js";import"./index-CWeydB1c.js";const ir=`<template>\r
...
import { CustomGUI } from "@/utils/gui";\r
\r
...
`,ar={class:"box",ref:"viewerRef"},or=tr({name:"AdjustPositionAndScale"}),sr=Object.assign(or,{setup(lr){const f=W([{fileName:"@/views/3dtile/adjustPositionAndScale/index.vue",rawCode:ir,language:"html"},{fileName:"@/utils/cesium.js",rawCode:P,language:"javascript"}]),C=$("viewerRef");let i=null,a=null,c=null,t=null,u=null;const n={longitude:0,latitude:0,height:0,rotateX:0,rotateY:0,rotateZ:0,scale:1,reset:()=>{if(u){const e=r.Cartographic.fromCartesian(u);n.longitude=Number(r.Math.toDegrees(e.longitude).toFixed(6)),n.latitude=Number(r.Math.toDegrees(e.latitude).toFixed(6))}n.height=0,n.rotateX=0,n.rotateY=0,n.rotateZ=0,n.scale=1,o(),a&&i&&i.zoomTo(a,new r.HeadingPitchRange(0,-.5,a.boundingSphere.radius*2)),t&&t.controllersRecursive().forEach(e=>e.updateDisplay())}};Q(()=>{c=setTimeout(()=>{h()},0)});function x(e,s={}){if(!e)return new r.Matrix4;const{longitude:l,latitude:p,height:w=0,rotateX:v=0,rotateY:b=0,rotateZ:N=0,scale:R=1}=s,d=r.Cartographic.fromCartesian(e),D=r.Math.toDegrees(d.longitude),_=r.Math.toDegrees(d.latitude),y=d.height,E=l!==void 0?Number(l):D,S=p!==void 0?Number(p):_,U=y+(Number(w)||0),F=r.Cartesian3.fromDegrees(E,S,U),L=r.Transforms.eastNorthUpToFixedFrame(F,r.Ellipsoid.WGS84,new r.Matrix4),Z=r.Transforms.eastNorthUpToFixedFrame(e,r.Ellipsoid.WGS84,new r.Matrix4),X=r.Matrix4.inverse(Z,new r.Matrix4),Y=r.Matrix3.fromRotationX(r.Math.toRadians(Number(v)||0)),A=r.Matrix3.fromRotationY(r.Math.toRadians(Number(b)||0)),B=r.Matrix3.fromRotationZ(r.Math.toRadians(Number(N)||0));let m=r.Matrix3.multiply(B,A,new r.Matrix3);m=r.Matrix3.multiply(m,Y,new r.Matrix3);const g=Number(R)||1,j=r.Matrix3.fromScale(new r.Cartesian3(g,g,g),new r.Matrix3);m=r.Matrix3.multiply(m,j,new r.Matrix3);const O=r.Matrix4.fromRotationTranslation(m,r.Cartesian3.ZERO,new r.Matrix4),z=r.Matrix4.multiply(L,O,new r.Matrix4);return r.Matrix4.multiply(z,X,new r.Matrix4)}function M(e,s,l){!e||!s||(e.modelMatrix=x(s,l))}function o(){M(a,u,n)}async function h(){i=G(C.value,{shadows:!0}),I(i,{msaaSamples:4,enableFxaa:!0}),i.scene.globe.depthTestAgainstTerrain=!0;try{a=await r.Cesium3DTileset.fromUrl("/gisnotes-cs/SampleData/Cesium3DTiles/Tilesets/Tileset/tileset.json"),i.scene.primitives.add(a),u=a.boundingSphere.center.clone();const e=r.Cartographic.fromCartesian(u);n.longitude=Number(r.Math.toDegrees(e.longitude).toFixed(6)),n.latitude=Number(r.Math.toDegrees(e.latitude).toFixed(6)),i.zoomTo(a,new r.HeadingPitchRange(0,-.5,a.boundingSphere.radius*2)),o()}catch(e){console.error(`tileset加载失败: ${e}`)}T()}function T(){t=new V({container:C.value,title:"模型位姿与缩放调节"});const e=r.Cartographic.fromCartesian(u),s=r.Math.toDegrees(e.longitude),l=r.Math.toDegrees(e.latitude);t.add(n,"longitude",s-.02,s+.02,1e-5).name("经度 (°)").onChange(()=>o()),t.add(n,"latitude",l-.02,l+.02,1e-5).name("纬度 (°)").onChange(()=>o()),t.add(n,"height",-100,100,1).name("高度偏移 (米)").onChange(()=>o()),t.add(n,"rotateX",-180,180,1).name("绕 X 轴旋转 (°)").onChange(()=>o()),t.add(n,"rotateY",-180,180,1).name("绕 Y 轴旋转 (°)").onChange(()=>o()),t.add(n,"rotateZ",-180,180,1).name("绕 Z 轴旋转 (°)").onChange(()=>o()),t.add(n,"scale",.1,5,.1).name("模型缩放 (倍数)").onChange(()=>o()),t.add(n,"reset").name("重置")}return q(()=>{c&&clearTimeout(c),t&&(t.destroy(),t=null),i&&(i.destroy(),i=null)}),(e,s)=>(J(),K(k,{codeBlocks:nr(f)},{default:rr(()=>[er("div",ar,null,512)]),_:1},8,["codeBlocks"]))}}),Cr=H(sr,[["__scopeId","data-v-caed7c57"]]);export{Cr as default};
