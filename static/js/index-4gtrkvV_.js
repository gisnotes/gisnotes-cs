import{c as z,o as O,C as e,D as P}from"./cesium-C3e13GYG.js";import{C as k}from"./cesium-pNEfRpPT.js";import{M as V}from"./datGUI-BDf2v8uN.js";import{_ as H,r as W,T as Q,F as $,M as q,o as J,m as K,f as ee,h as re,i as te,H as ne}from"./index-DuSIVT5r.js";import"./index-CTKpwe3s.js";const ie=`<template>\r
...
\r
  ...
  try {\r
    tileset = await Cesium.Cesium3DTileset.fromUrl(\r
      "/SampleData/Cesium3DTiles/Tilesets/Tileset/tileset.json",\r
    );\r
\r
...
`,ae={class:"box",ref:"viewerRef"},oe=ne({name:"AdjustPositionAndScale"}),se=Object.assign(oe,{setup(le){const f=W([{fileName:"@/views/3dtile/adjustPositionAndScale/index.vue",rawCode:ie,language:"html"},{fileName:"@/utils/cesium.js",rawCode:k,language:"javascript"}]),C=Q("viewerRef");let i=null,a=null,d=null,n=null,u=null;const r={longitude:0,latitude:0,height:0,rotateX:0,rotateY:0,rotateZ:0,scale:1,reset:()=>{if(u){const t=e.Cartographic.fromCartesian(u);r.longitude=Number(e.Math.toDegrees(t.longitude).toFixed(6)),r.latitude=Number(e.Math.toDegrees(t.latitude).toFixed(6))}r.height=0,r.rotateX=0,r.rotateY=0,r.rotateZ=0,r.scale=1,o(),a&&i&&i.zoomTo(a,new e.HeadingPitchRange(0,-.5,a.boundingSphere.radius*2)),n&&n.updateDisplay()}};$(()=>{d=setTimeout(()=>{h()},0)});function M(t,s={}){if(!t)return new e.Matrix4;const{longitude:l,latitude:p,height:T=0,rotateX:b=0,rotateY:v=0,rotateZ:D=0,scale:R=1}=s,c=e.Cartographic.fromCartesian(t),y=e.Math.toDegrees(c.longitude),N=e.Math.toDegrees(c.latitude),_=c.height,S=l!==void 0?Number(l):y,U=p!==void 0?Number(p):N,L=_+(Number(T)||0),F=e.Cartesian3.fromDegrees(S,U,L),Z=e.Transforms.eastNorthUpToFixedFrame(F,e.Ellipsoid.WGS84,new e.Matrix4),X=e.Transforms.eastNorthUpToFixedFrame(t,e.Ellipsoid.WGS84,new e.Matrix4),Y=e.Matrix4.inverse(X,new e.Matrix4),j=e.Matrix3.fromRotationX(e.Math.toRadians(Number(b)||0)),A=e.Matrix3.fromRotationY(e.Math.toRadians(Number(v)||0)),B=e.Matrix3.fromRotationZ(e.Math.toRadians(Number(D)||0));let m=e.Matrix3.multiply(B,A,new e.Matrix3);m=e.Matrix3.multiply(m,j,new e.Matrix3);const g=Number(R)||1,E=e.Matrix3.fromScale(new e.Cartesian3(g,g,g),new e.Matrix3);m=e.Matrix3.multiply(m,E,new e.Matrix3);const G=e.Matrix4.fromRotationTranslation(m,e.Cartesian3.ZERO,new e.Matrix4),I=e.Matrix4.multiply(Z,G,new e.Matrix4);return e.Matrix4.multiply(I,Y,new e.Matrix4)}function x(t,s,l){!t||!s||(t.modelMatrix=M(s,l))}function o(){x(a,u,r)}async function h(){i=z(C.value,{shadows:!0}),O(i,{msaaSamples:4,enableFxaa:!0}),i.scene.globe.depthTestAgainstTerrain=!0;try{a=await e.Cesium3DTileset.fromUrl("/SampleData/Cesium3DTiles/Tilesets/Tileset/tileset.json"),i.scene.primitives.add(a),u=a.boundingSphere.center.clone();const t=e.Cartographic.fromCartesian(u);r.longitude=Number(e.Math.toDegrees(t.longitude).toFixed(6)),r.latitude=Number(e.Math.toDegrees(t.latitude).toFixed(6)),i.zoomTo(a,new e.HeadingPitchRange(0,-.5,a.boundingSphere.radius*2)),o()}catch(t){console.error(`tileset加载失败: ${t}`)}w()}function w(){n=new V,n.modifyPosition(C.value);const t=e.Cartographic.fromCartesian(u),s=e.Math.toDegrees(t.longitude),l=e.Math.toDegrees(t.latitude);n.add(r,"longitude",s-.02,s+.02,1e-5).name("经度 (°)").onChange(()=>o()),n.add(r,"latitude",l-.02,l+.02,1e-5).name("纬度 (°)").onChange(()=>o()),n.add(r,"height",-100,100,1).name("高度偏移 (米)").onChange(()=>o()),n.add(r,"rotateX",-180,180,1).name("绕 X 轴旋转 (°)").onChange(()=>o()),n.add(r,"rotateY",-180,180,1).name("绕 Y 轴旋转 (°)").onChange(()=>o()),n.add(r,"rotateZ",-180,180,1).name("绕 Z 轴旋转 (°)").onChange(()=>o()),n.add(r,"scale",.1,5,.1).name("模型缩放 (倍数)").onChange(()=>o()),n.add(r,"reset").name("重置")}return q(()=>{d&&clearTimeout(d),n&&(n.destroy(),n=null),i&&(i.destroy(),i=null)}),(t,s)=>(J(),K(P,{codeBlocks:te(f)},{default:ee(()=>[re("div",ae,null,512)]),_:1},8,["codeBlocks"]))}}),Ce=H(se,[["__scopeId","data-v-9d069fd8"]]);export{Ce as default};
